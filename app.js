...
function exportGames() {
  ...
  link.href = url;
  link.download = `goalietrack-backup-${new Date().toISOString().slice(0, 10)}.json`;
  document.body.appendChild(link);
  link.click();
  link.remove();
  URL.revokeObjectURL(url);
  setStatus("Backup exported.", "valid");
}
...
function importGamesFromFile(file) {
  ...
  reader.onload = () => {
    try {
      ...
      games = importedGames.filter((game) =>
        game && Number.isFinite(Number(game.id)) && typeof game.opponent === "string",
      );
      saveGames();
      currentGameId = null;
      clearDraft();
      setDefaultValues();
      renderAll();
      setStatus(`Imported ${games.length} game(s).`, "valid");
    } catch {
      setStatus("Import failed. Please choose a valid goalietrack backup file.", "invalid");
    } finally {
...
refs.exportBtn.addEventListener("click", exportGames);

refs.importFile.addEventListener("change", (event) => {
  const target = event.target;
  if (!(target instanceof HTMLInputElement)) return;
  importGamesFromFile(target.files?.[0]);
});
